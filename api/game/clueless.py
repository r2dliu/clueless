...
class Clueless:
    ...
    def __init__(self, *args, **kwargs):
        self.rooms = ['study', 'hall', 'lounge', 'library', 'billiard',
                      'dining', 'conservatory', 'ballroom', 'kitchen']
        self.suspects = ['colonel_mustard', 'miss_scarlet', 'professor_plum',
                         'mr_green', 'mrs_white', 'mrs_peacock']
        self.weapons = ['rope', 'lead_pipe', 'knife', 'wrench', 
                        'candlestick', 'revolver']
        self.cards  = self.suspects + self.weapons + self.rooms
        
        self.hallways = ['study_hall', 'hall_lounge', 'lounge_dining',
                         'dining_kitchen', 'kitchen_ballroom', 
                         'ballroom_conservatory', 'conservatory_library',
                         'library_study', 'hall_billiard', 'dining_billiard',
                         'ballroom_billiard', 'library_billiard']
        self.secret_passages = ['study_kitchen', 'lounge_conservatory']

        # players dict holds client_id: character assignments
        self.players = {}
        self.state = initialize_board(players)
